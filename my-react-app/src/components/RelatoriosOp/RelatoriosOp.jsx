...
const classificarTransfer = (item) => {
    ...
    if (serviceType === 1 || origem.includes("aero") || nome.includes("in ")) return TRANSFER_TIPOS.IN;
    if (serviceType === 2 || destino.includes("aero") || nome.includes("out ")) return TRANSFER_TIPOS.OUT;
...
const construirChaveServico = (item) => {
    const data = extrairDataServico(item);
    const horario = extrairHorarioServico(item);
    const guia = extrairGuiaApi(item) || "SEM_GUIA";
    const servico = obterNomeCanonico(extrairNomePasseio(item) || classificarTransfer(item));
    const origem = extrairOrigem(item);
    const destino = extrairDestino(item);
    const reserva = item?.reserve_id || item?.reserve?.id || item?.id || "SEM_ID";

    return [data, horario, guia, servico, origem, destino, reserva]
        .map((parte) => normalizarTexto(String(parte || "")))
        .join("|");
};

const deduplicarServicos = (lista = []) => {
    const mapa = new Map();
    lista.forEach((item) => {
        const chave = construirChaveServico(item);
    ...
    return (
        <div style={tooltipStyle}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6, maxWidth: 360 }}>
                {detalhes.slice(0, 10).map((item, index) => (
                    <div key={`${item.guia}_${item.servico}_${index}`} style={{ fontSize: 11 }}>
...
const RelatoriosOperacionaisApiOnlyCorrigido = () => {
    ...
    const [filtroTransferRegiao, setFiltroTransferRegiao] = useState(REGIOES.TODAS);
    ...
    const carregarDados = useCallback(async ({ silent = false } = {}) => {
        try {
            ...
            const periodoComparativo = getPeriodoComparativo(periodoInicio, periodoFim);
            const datasAtuais = listarDatasNoPeriodo(periodoInicio, periodoFim);
            const datasComparativas = listarDatasNoPeriodo(periodoComparativo.inicio, periodoComparativo.fim);
    ...
    const apiAtualUnica = useMemo(() => deduplicarServicos(apiAtual), [apiAtual]);
    const apiComparativaUnica = useMemo(() => deduplicarServicos(apiComparativa), [apiComparativa]);

    const resumoComparativo = useMemo(() => {
        const totalPaxAtual = apiAtualUnica.reduce((acc, item) => acc + Number(extrairContagemPax(item).total || 0), 0);
        const totalPaxAnterior = apiComparativaUnica.reduce((acc, item) => acc + Number(extrairContagemPax(item).total || 0), 0);
        const servicosAtual = apiAtualUnica.length;
        const servicosAnterior = apiComparativaUnica.length;

        return {
            paxAtual: totalPaxAtual,
            paxAnterior: totalPaxAnterior,
            deltaPax: totalPaxAtual - totalPaxAnterior,
            deltaPaxPercentual: calcularDeltaPercentual(totalPaxAtual, totalPaxAnterior),
            servicosAtual,
            servicosAnterior,
            deltaServicos: servicosAtual - servicosAnterior,
            deltaServicosPercentual: calcularDeltaPercentual(servicosAtual, servicosAnterior),
        };
    }, [apiAtualUnica, apiComparativaUnica]);

    const timelineDemanda = useMemo(() => {
        const mapa = {};
        apiAtualUnica.forEach((item) => {
            const date = extrairDataServico(item);
            if (!date) return;
            if (!mapa[date]) {
                mapa[date] = { date, label: formatarDataBr(date), pax: 0, servicos: 0, passeios: 0, transfers: 0 };
            }
            mapa[date].pax += Number(extrairContagemPax(item).total || 0);
            mapa[date].servicos += 1;
            if (ehPasseio(item)) mapa[date].passeios += 1;
            else mapa[date].transfers += 1;
        });
        return Object.values(mapa).sort((a, b) => String(a.date).localeCompare(String(b.date)));
    }, [apiAtualUnica]);

    const timelineGuias = useMemo(() => {
        const mapa = {};
        apiAtualUnica.forEach((item) => {
            const guia = extrairGuiaApi(item);
            if (!guia) return;
            const data = extrairDataServico(item);
            if (!data) return;
            if (!mapa[data]) {
                mapa[data] = { date: data, label: formatarDataBr(data), servicos: 0, detalhes: [] };
            }
            mapa[data].servicos += 1;
            mapa[data].detalhes.push({
                guia,
                servico: obterNomeCanonico(extrairNomePasseio(item) || classificarTransfer(item)),
    ...
    }, [apiAtualUnica, periodoFim, periodoInicio]);

    const guiasReport = useMemo(() => {
        const mapa = new Map();
        apiAtualUnica.forEach((item) => {
            const nomeGuia = extrairGuiaApi(item);
            if (!nomeGuia) return;
            const date = extrairDataServico(item);
            const horario = extrairHorarioServico(item);
            const serviceName = obterNomeCanonico(extrairNomePasseio(item) || classificarTransfer(item));
            const tipo = ehPasseio(item) ? "PASSEIO" : classificarTransfer(item);
            const veiculo = extrairVeiculo(item);

            if (!mapa.has(nomeGuia)) mapa.set(nomeGuia, { id: nomeGuia, nome: nomeGuia, detalhes: [] });
            mapa.get(nomeGuia).detalhes.push({ data: date, horario, servico: serviceName, tipo, veiculo });
        });

        return Array.from(mapa.values())
            .map((item) => {
                const detalhes = item.detalhes.sort((a, b) => `${a.data} ${a.horario}`.localeCompare(`${b.data} ${b.horario}`));
                return {
                    ...item,
                    detalhes,
                    totalServicos: detalhes.length,
                    passeios: detalhes.filter((d) => d.tipo === "PASSEIO").length,
                    inCount: detalhes.filter((d) => d.tipo === TRANSFER_TIPOS.IN).length,
                    outCount: detalhes.filter((d) => d.tipo === TRANSFER_TIPOS.OUT).length,
                    transferCount: detalhes.filter((d) => d.tipo === TRANSFER_TIPOS.TRANSFER).length,
                    diasAtivos: new Set(detalhes.map((d) => d.data)).size,
                    totalVeiculos: new Set(detalhes.map((d) => d.veiculo)).size,
                };
            })
            .sort((a, b) => b.totalServicos - a.totalServicos || a.nome.localeCompare(b.nome, "pt-BR", { sensitivity: "base" }));
    }, [apiAtualUnica]);

    const passeiosBase = useMemo(() => {
        const mapa = new Map();
        apiAtualUnica.filter((item) => ehPasseio(item)).forEach((item) => {
            const nome = obterNomeCanonico(extrairNomePasseio(item));
            if (!nome || deveIgnorarServico(nome)) return;
            const data = extrairDataServico(item);
            const pax = Number(extrairContagemPax(item).total || 0);
            const operadora = extrairOperadora(item);
            const guia = extrairGuiaApi(item) || "Sem guia";
            const horario = extrairHorarioServico(item);
            if (!mapa.has(nome)) mapa.set(nome, { nome, detalhes: [] });
            mapa.get(nome).detalhes.push({ date: data, horario, pax, operadora, guia });
        });

        return Array.from(mapa.values())
            .map((item) => {
                const detalhes = item.detalhes.sort((a, b) => `${a.date} ${a.horario}`.localeCompare(`${b.date} ${b.horario}`));
                const porDia = {};
                detalhes.forEach((detalhe) => {
                    if (!porDia[detalhe.date]) porDia[detalhe.date] = { date: detalhe.date, label: formatarDataBr(detalhe.date), pax: 0, servicos: 0 };
                    porDia[detalhe.date].pax += Number(detalhe.pax || 0);
                    porDia[detalhe.date].servicos += 1;
                });
                const timeline = Object.values(porDia).sort((a, b) => String(a.date).localeCompare(String(b.date)));
                const diasComGrupo = timeline.filter((dia) => dia.pax >= 8).length;
                const diasSemGrupo = timeline.filter((dia) => dia.pax < 8).length;
                return {
                    ...item,
                    detalhes,
                    ocorrencias: detalhes.length,
                    diasOperados: timeline.length,
                    pax: detalhes.reduce((acc, d) => acc + Number(d.pax || 0), 0),
                    diasComGrupo,
                    diasSemGrupo,
                    taxaFormacao: timeline.length > 0 ? Math.round((diasComGrupo / timeline.length) * 100) : 0,
                    timeline,
                };
            })
            .sort((a, b) => b.pax - a.pax || a.nome.localeCompare(b.nome, "pt-BR", { sensitivity: "base" }));
    }, [apiAtualUnica]);

    const transfersBase = useMemo(() => {
        return apiAtualUnica
            .filter((item) => !ehPasseio(item))
            .map((item, index) => {
                const tipo = classificarTransfer(item);
                return {
                    id: `${extrairDataServico(item)}_${index}_${tipo}`,
                    date: extrairDataServico(item),
                    horario: extrairHorarioServico(item),
                    tipo,
                    ...
                    pax: Number(extrairContagemPax(item).total || 0),
                    guia: extrairGuiaApi(item) || "Sem guia",
                    veiculo: extrairVeiculo(item),
                    serviceName: obterNomeCanonico(extrairNomePasseio(item) || tipo),
                };
            })
            .sort((a, b) => `${a.date} ${a.horario}`.localeCompare(`${b.date} ${b.horario}`));
    }, [apiAtualUnica]);

    const opcoesGuia = useMemo(() => ["TODOS", ...guiasReport.map((item) => item.nome)], [guiasReport]);
    const opcoesPasseiosDeGuias = useMemo(() => ["TODOS", ...Array.from(new Set(guiasReport.flatMap((item) => item.detalhes.map((d) => d.servico)))).sort()], [guiasReport]);
    const opcoesVeiculosGuias = useMemo(() => ["TODOS", ...Array.from(new Set(guiasReport.flatMap((item) => item.detalhes.map((d) => d.veiculo)))).sort()], [guiasReport]);
    const opcoesPasseios = useMemo(() => ["TODOS", ...passeiosBase.map((item) => item.nome)], [passeiosBase]);
    const opcoesOperadorasPasseio = useMemo(() => ["TODAS", ...Array.from(new Set(passeiosBase.flatMap((item) => item.detalhes.map((d) => d.operadora)))).sort()], [passeiosBase]);
    const opcoesOperadorasTransfer = useMemo(() => ["TODAS", ...Array.from(new Set(transfersBase.map((item) => item.operadora))).sort()], [transfersBase]);
    ...
    const guiasFiltrados = useMemo(() => {
        return guiasReport
            .map((guia) => {
                const detalhesFiltrados = guia.detalhes.filter((detalhe) => {
                    const matchBusca =
                        ...
                        normalizarTexto(detalhe.servico).includes(normalizarTexto(filtroBusca)) ||
                        normalizarTexto(detalhe.veiculo).includes(normalizarTexto(filtroBusca));
                    const matchGuia = filtroGuia === "TODOS" || guia.nome === filtroGuia;
                    const matchPasseio = filtroPasseioGuia === "TODOS" || detalhe.servico === filtroPasseioGuia;
                    const matchVeiculo = filtroVeiculoGuia === "TODOS" || detalhe.veiculo === filtroVeiculoGuia;
                ...
                return {
                    ...
                    passeios: detalhesFiltrados.filter((d) => d.tipo === "PASSEIO").length,
                    inCount: detalhesFiltrados.filter((d) => d.tipo === TRANSFER_TIPOS.IN).length,
                    outCount: detalhesFiltrados.filter((d) => d.tipo === TRANSFER_TIPOS.OUT).length,
                    transferCount: detalhesFiltrados.filter((d) => d.tipo === TRANSFER_TIPOS.TRANSFER).length,
            ...
            .sort((a, b) => b.totalServicos - a.totalServicos || a.nome.localeCompare(b.nome, "pt-BR", { sensitivity: "base" }));
    }, [filtroBusca, filtroGuia, filtroPasseioGuia, filtroVeiculoGuia, guiasReport]);
    ...
    const passeiosFiltrados = useMemo(() => {
        return passeiosBase
            .map((item) => {
                const detalhesFiltrados = item.detalhes.filter((detalhe) => {
                    const matchBusca =
                        ...
                        normalizarTexto(detalhe.operadora).includes(normalizarTexto(filtroBusca)) ||
                        normalizarTexto(detalhe.guia).includes(normalizarTexto(filtroBusca));
                    const matchPasseio = filtroPasseio === "TODOS" || item.nome === filtroPasseio;
                    const matchOperadora = filtroOperadoraPasseio === "TODAS" || detalhe.operadora === filtroOperadoraPasseio;
                ...
                return {
                    ...item,
                    detalhes: detalhesFiltrados,
                    ocorrencias: detalhesFiltrados.length,
                    diasOperados: timeline.length,
                    pax: detalhesFiltrados.reduce((acc, d) => acc + Number(d.pax || 0), 0),
                    diasComGrupo,
                    diasSemGrupo,
                    taxaFormacao: timeline.length > 0 ? Math.round((diasComGrupo / timeline.length) * 100) : 0,
                    timeline,
            ...
            .filter((item) => !mostrarSomenteSemGrupo || item.diasSemGrupo > 0)
            .sort((a, b) => b.pax - a.pax || a.nome.localeCompare(b.nome, "pt-BR", { sensitivity: "base" }));
    }, [filtroBusca, filtroOperadoraPasseio, filtroPasseio, mostrarSomenteSemGrupo, passeiosBase]);

    const transfersFiltrados = useMemo(() => {
        return transfersBase.filter((item) => {
            const matchBusca =
                !filtroBusca ||
                normalizarTexto(item.serviceName).includes(normalizarTexto(filtroBusca)) ||
                normalizarTexto(item.operadora).includes(normalizarTexto(filtroBusca)) ||
            ...
            const matchTipo = filtroTransferTipo === "TODOS" || item.tipo === filtroTransferTipo;
            const matchRegiao = filtroTransferRegiao === REGIOES.TODAS || item.regiao === filtroTransferRegiao;
            const matchOperadora = filtroOperadora === "TODAS" || item.operadora === filtroOperadora;
            return matchBusca && matchTipo && matchRegiao && matchOperadora;
        });
    }, [filtroBusca, filtroOperadora, filtroTransferRegiao, filtroTransferTipo, transfersBase]);
    ...
    const graficoPasseios = useMemo(() => {
        return passeiosFiltrados.slice(0, 12).map((item) => ({
            ...
            pax: item.pax,
            diasComGrupo: item.diasComGrupo,
            diasSemGrupo: item.diasSemGrupo,
        }));
    }, [passeiosFiltrados]);
    ...
    const graficoTransfersOperadora = useMemo(() => {
        ...
        transfersFiltrados.forEach((item) => {
            if (!mapa[item.operadora]) mapa[item.operadora] = { operadora: item.operadora, IN: 0, OUT: 0, TRANSFER: 0 };
            mapa[item.operadora][item.tipo] += Number(item.pax || 0);
        });
        return Object.values(mapa).sort((a, b) => b.IN + b.OUT + b.TRANSFER - (a.IN + a.OUT + a.TRANSFER)).slice(0, 12);
    }, [transfersFiltrados]);
        ...
        return [
            {
                ...
                subtitulo: `${formatarDelta(resumoComparativo.deltaPax)} (${formatarDelta(resumoComparativo.deltaPaxPercentual)}%) vs período anterior`,
                destaque: resumoComparativo.deltaPax > 0 ? "positivo" : resumoComparativo.deltaPax < 0 ? "negativo" : "neutro",
            },
            {
                titulo: "Serviços no período",
                valor: formatarNumero(resumoComparativo.servicosAtual),
                subtitulo: `${formatarDelta(resumoComparativo.deltaServicos)} (${formatarDelta(resumoComparativo.deltaServicosPercentual)}%) vs período anterior`,
                destaque: resumoComparativo.deltaServicos > 0 ? "positivo" : resumoComparativo.deltaServicos < 0 ? "negativo" : "neutro",
            },
            {
            ...
            },
            {
                ...
                valor: formatarNumero(passeiosBase.reduce((acc, item) => acc + item.diasSemGrupo, 0)),
                subtitulo: "Dias sem formação no período",
                destaque: passeiosBase.some((item) => item.diasSemGrupo > 0) ? "alerta" : "positivo",
            },
        ];
        ...
        <div className="home-page relatorios-page">
            <div className="home-page-header relatorios-header">
                <div>
                    ...
                    <p className="home-page-subtitle">
                        Visão 100% Phoenix com filtros por período, guias, passeios e transfers.
                    </p>
                </div>

                <div className="home-page-actions">
                    <button type="button" className="home-refresh-btn" onClick={() => carregarDados({ silent: true })} disabled={loadingAtualizar || loading}>
                        {loadingAtualizar ? <SyncRounded className="spin" fontSize="small" /> : <RefreshRounded fontSize="small" />}
            ...
            <div className="home-dashboard-card home-dashboard-card-full relatorios-filtros-card">
                <div className="home-dashboard-card-header">
                    ...
                    <button type="button" className="home-collapse-btn" onClick={() => setFiltrosAbertos((prev) => !prev)}>
                        {filtrosAbertos ? <KeyboardArrowUpRounded /> : <KeyboardArrowDownRounded />}
                    </button>
                </div>
                    ...
                    <>
                        <div className="home-filters-grid relatorios-filtros-grid">
                            <label className="home-filter-field">
                                <span><CalendarMonthRounded fontSize="small" /> Início</span>
                                <input type="date" value={periodoInicio} max={periodoFim} onChange={(e) => setPeriodoInicio(e.target.value)} />
                            </label>

                            <label className="home-filter-field">
                                <span><CalendarMonthRounded fontSize="small" /> Fim</span>
                                <input type="date" value={periodoFim} min={periodoInicio} onChange={(e) => setPeriodoFim(e.target.value)} />
                            </label>

                            <label className="home-filter-field">
                                <span><SearchRounded fontSize="small" /> Busca geral</span>
                                <input type="text" placeholder="Guia, passeio, operadora, veículo..." value={filtroBusca} onChange={(e) => setFiltroBusca(e.target.value)} />
                            </label>

                            <div className="relatorios-range-shortcuts">
                                {rangeOptions.map((option) => (
                                    <button key={option.dias} type="button" className="home-chip-btn" onClick={() => {
                        ...
                        <div className="relatorios-tabs-nav">
                            <button type="button" className={`relatorios-tab-btn ${abaAtiva === ABAS.GUIAS ? "active" : ""}`} onClick={() => setAbaAtiva(ABAS.GUIAS)}>
                                <ManageAccountsRounded fontSize="small" /> Guias
                            </button>
                            <button type="button" className={`relatorios-tab-btn ${abaAtiva === ABAS.PASSEIOS ? "active" : ""}`} onClick={() => setAbaAtiva(ABAS.PASSEIOS)}>
                                <TravelExploreRounded fontSize="small" /> Passeios
                            </button>
                            <button type="button" className={`relatorios-tab-btn ${abaAtiva === ABAS.TRANSFERS ? "active" : ""}`} onClick={() => setAbaAtiva(ABAS.TRANSFERS)}>
                        ...
                        <div className="home-filters-grid relatorios-filtros-grid relatorios-filtros-especificos">
                            {abaAtiva === ABAS.GUIAS && (
                                <>
                                    <label className="home-filter-field">
                                        <span><ManageAccountsRounded fontSize="small" /> Guia</span>
                                        <select value={filtroGuia} onChange={(e) => setFiltroGuia(e.target.value)}>
                                            {opcoesGuia.map((item) => <option key={item} value={item}>{item}</option>)}
                                        </select>
                                    </label>

                                    <label className="home-filter-field">
                                        <span><TravelExploreRounded fontSize="small" /> Serviço</span>
                                        <select value={filtroPasseioGuia} onChange={(e) => setFiltroPasseioGuia(e.target.value)}>
                                            {opcoesPasseiosDeGuias.map((item) => <option key={item} value={item}>{item}</option>)}
                                        </select>
                                    </label>

                                    <label className="home-filter-field">
                                        <span><RouteRounded fontSize="small" /> Veículo</span>
                                        <select value={filtroVeiculoGuia} onChange={(e) => setFiltroVeiculoGuia(e.target.value)}>
                                            {opcoesVeiculosGuias.map((item) => <option key={item} value={item}>{item}</option>)}
                                        </select>
                                    </label>
                                </>
                            )}
                            ...
                            {abaAtiva === ABAS.PASSEIOS && (
                                <>
                                    <label className="home-filter-field">
                                        <span><TravelExploreRounded fontSize="small" /> Passeio</span>
                                        <select value={filtroPasseio} onChange={(e) => setFiltroPasseio(e.target.value)}>
                                            {opcoesPasseios.map((item) => <option key={item} value={item}>{item}</option>)}
                                        </select>
                                    </label>

                                    <label className="home-filter-field">
                                        <span><GroupsRounded fontSize="small" /> Operadora</span>
                                        <select value={filtroOperadoraPasseio} onChange={(e) => setFiltroOperadoraPasseio(e.target.value)}>
                                            {opcoesOperadorasPasseio.map((item) => <option key={item} value={item}>{item}</option>)}
                                        </select>
                                    </label>

                                    <label className="home-filter-checkbox">
                                        <input type="checkbox" checked={mostrarSomenteSemGrupo} onChange={(e) => setMostrarSomenteSemGrupo(e.target.checked)} />
                                        <span>Somente passeios com dias sem grupo</span>
                                    </label>
                                </>
                            )}
                            ...
                            {abaAtiva === ABAS.TRANSFERS && (
                                <>
                                    <label className="home-filter-field">
                                        <span><CompareArrowsRounded fontSize="small" /> Tipo</span>
                                        <select value={filtroTransferTipo} onChange={(e) => setFiltroTransferTipo(e.target.value)}>
                                    ...
                                    <label className="home-filter-field">
                                        <span><FlightLandRounded fontSize="small" /> Região</span>
                                        <select value={filtroTransferRegiao} onChange={(e) => setFiltroTransferRegiao(e.target.value)}>
                                    ...
                                    <label className="home-filter-field">
                                        <span><GroupsRounded fontSize="small" /> Operadora</span>
                                        <select value={filtroOperadora} onChange={(e) => setFiltroOperadora(e.target.value)}>
                                            {opcoesOperadorasTransfer.map((item) => <option key={item} value={item}>{item}</option>)}
                                        </select>
                                    </label>
                                </>
                            )}
                ...
                {abaAtiva === ABAS.GUIAS && (
                    <>
                        <div className="home-dashboard-card home-dashboard-card-large">
                            ...
                            {loading ? renderCardLoading("Carregando linha do tempo de guias...") : (
                                <div style={{ width: "100%", height: 340 }}>
                                    <ResponsiveContainer>
                                        <LineChart data={timelineGuias} margin={{ top: 10, right: 20, left: 0, bottom: 10 }}>
                                            <CartesianGrid strokeDasharray="3 3" opacity={0.18} />
                                            <XAxis dataKey="label" />
                                            <YAxis allowDecimals={false} />
                                            <Tooltip content={<TooltipTimelineGuias />} />
                                            <Legend />
                                            <Line type="monotone" dataKey="servicos" name="Serviços com guia" stroke={CHART_COLORS[0]} strokeWidth={3} dot={{ r: 3 }} />
                                        </LineChart>
                                    </ResponsiveContainer>
                                </div>
                            )}
                        ...
                        <div className="home-dashboard-card home-dashboard-card-full">
                            ...
                            {loading ? renderCardLoading("Carregando balanço de guias...") : (
                                    ...
                                    <table className="home-table">
                                        ...
                                        <tbody>
                                            {guiasFiltrados.length === 0 ? (
                                                <tr>
                                                    <td colSpan={8} className="empty-state">Nenhum guia encontrado no Phoenix para os filtros selecionados.</td>
                                                </tr>
                                            ) : (
                ...
                {abaAtiva === ABAS.PASSEIOS && (
                    <>
                        <div className="home-dashboard-card home-dashboard-card-large">
                            ...
                            {loading ? renderCardLoading("Carregando gráfico de passeios...") : (
                                <div style={{ width: "100%", height: 360 }}>
                                    <ResponsiveContainer>
                                        <ComposedChart data={graficoPasseios} layout="vertical" margin={{ top: 10, right: 20, left: 55, bottom: 10 }}>
                                            <CartesianGrid strokeDasharray="3 3" opacity={0.18} />
                                            <XAxis type="number" allowDecimals={false} />
                                            <YAxis type="category" dataKey="nome" width={180} />
                                            <Tooltip contentStyle={tooltipStyle} />
                                            <Legend />
                                            <Bar dataKey="diasComGrupo" name="Dias com grupo" fill={CHART_COLORS[2]} radius={[0, 8, 8, 0]} />
                                            <Bar dataKey="diasSemGrupo" name="Dias sem grupo" fill={CHART_COLORS[6]} radius={[0, 8, 8, 0]} />
                                            <Line type="monotone" dataKey="pax" name="Pax no período" stroke={CHART_COLORS[0]} strokeWidth={3} dot={{ r: 3 }} />
                                        </ComposedChart>
                                    </ResponsiveContainer>
                                </div>
                            )}
                        ...
                        <div className="home-dashboard-card home-dashboard-card-large">
                            ...
                            {loading ? renderCardLoading("Carregando linha do tempo...") : (
                                <div style={{ width: "100%", height: 340 }}>
                                    <ResponsiveContainer>
                                        <LineChart data={passeiosFiltrados[0]?.timeline || []} margin={{ top: 10, right: 20, left: 0, bottom: 10 }}>
                                            <CartesianGrid strokeDasharray="3 3" opacity={0.18} />
                                            <XAxis dataKey="label" />
                                            <YAxis allowDecimals={false} />
                                            <Tooltip contentStyle={tooltipStyle} />
                                            <Legend />
                                            <Line type="monotone" dataKey="pax" name="Pax" stroke={CHART_COLORS[0]} strokeWidth={3} dot={{ r: 3 }} />
                                            <Line type="monotone" dataKey="servicos" name="Serviços" stroke={CHART_COLORS[4]} strokeWidth={3} dot={{ r: 3 }} />
                                        </LineChart>
                                    </ResponsiveContainer>
                                </div>
                            )}
                        ...
                        <div className="home-dashboard-card home-dashboard-card-full">
                            ...
                            {loading ? renderCardLoading("Carregando balanço de passeios...") : (
                                <div className="home-table-wrap">
                                    <table className="home-table">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Dias operados</th>
                                                <th>Pax</th>
                                                <th>Dias com grupo</th>
                                                <th>Dias sem grupo</th>
                                        ...
                                        <tbody>
                                            {passeiosFiltrados.length === 0 ? (
                                                <tr>
                                                    <td colSpan={7} className="empty-state">Nenhum passeio encontrado no Phoenix para os filtros selecionados.</td>
                                                </tr>
                                            ) : (
                                                passeiosFiltrados.map((item) => (
                                                    <tr key={item.nome}>
                                                        ...
                                                        <td>{item.diasComGrupo}</td>
                                                        <td>
                                                            <span className={`status-pill ${item.diasSemGrupo > 0 ? "alerta" : "ok"}`}>
                                                                {item.diasSemGrupo}
                                                            </span>
                                                        </td>
                ...
                {abaAtiva === ABAS.TRANSFERS && (
                    <>
                        <div className="home-dashboard-card home-dashboard-card-large">
                            ...
                            {loading ? renderCardLoading("Carregando comparativo...") : (
                                <div style={{ width: "100%", height: 320 }}>
                                    <ResponsiveContainer>
                                        <ComposedChart data={timelineDemanda} margin={{ top: 10, right: 20, left: 0, bottom: 10 }}>
                                            <CartesianGrid strokeDasharray="3 3" opacity={0.18} />
                                            <XAxis dataKey="label" />
                                            ...
                                            <Tooltip contentStyle={tooltipStyle} />
                                            <Legend />
                                            <Bar yAxisId="left" dataKey="pax" name="Pax" radius={[8, 8, 0, 0]} fill={CHART_COLORS[0]} />
                                            <Line yAxisId="right" type="monotone" dataKey="servicos" name="Serviços" stroke={CHART_COLORS[4]} strokeWidth={3} dot={{ r: 3 }} />
                                        </ComposedChart>
                                    </ResponsiveContainer>
                                </div>
                            )}
                        ...
                        <div className="home-dashboard-card home-dashboard-card-large">
                            ...
                            {loading ? renderCardLoading("Carregando operadoras...") : (
                                <div style={{ width: "100%", height: 340 }}>
                                    <ResponsiveContainer>
                                        <BarChart data={graficoTransfersOperadora} margin={{ top: 10, right: 20, left: 0, bottom: 10 }}>
                                            <CartesianGrid strokeDasharray="3 3" opacity={0.18} />
                                            <XAxis dataKey="operadora" angle={-20} textAnchor="end" interval={0} height={70} />
                                            <YAxis allowDecimals={false} />
                                            <Tooltip contentStyle={tooltipStyle} />
                                            <Legend />
                                            <Bar dataKey="IN" stackId="a" name="IN" fill={CHART_COLORS[0]} radius={[8, 8, 0, 0]} />
                                            <Bar dataKey="OUT" stackId="a" name="OUT" fill={CHART_COLORS[4]} radius={[8, 8, 0, 0]} />
                                            <Bar dataKey="TRANSFER" stackId="a" name="TRANSFER" fill={CHART_COLORS[2]} radius={[8, 8, 0, 0]} />
                                        </BarChart>
                                    </ResponsiveContainer>
                                </div>
                            )}
                        ...
                        <div className="home-dashboard-card home-dashboard-card-large">
                            ...
                            {loading ? renderCardLoading("Carregando mix operacional...") : (
                                <div style={{ width: "100%", height: 340 }}>
                                    <ResponsiveContainer>
                                        <PieChart>
                                            <Pie data={graficoTransfersMix} dataKey="value" nameKey="name" innerRadius={75} outerRadius={115} paddingAngle={3}>
                                                {graficoTransfersMix.map((entry, index) => <Cell key={entry.name} fill={CHART_COLORS[index % CHART_COLORS.length]} />)}
                                            </Pie>
                                            <Tooltip contentStyle={tooltipStyle} />
                                            <Legend />
                                        </PieChart>
                        ...
                        <div className="home-dashboard-card home-dashboard-card-full">
                            ...
                            {loading ? renderCardLoading("Carregando detalhamento de transfers...") : (
                                <div className="home-table-wrap">
                                    <table className="home-table">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            {transfersFiltrados.length === 0 ? (
                                                <tr>
                                                    <td colSpan={10} className="empty-state">Nenhum transfer encontrado no Phoenix para os filtros selecionados.</td>
                                                </tr>
                                            ) : (
                                                transfersFiltrados.map((item) => (
                                                    <tr key={item.id}>
                                                        ...
                                                        <td>{item.horario || "--:--"}</td>
                                                        <td><span className="status-pill neutro">{item.tipo}</span></td>
                                                        <td>{item.regiao}</td>
...
export default RelatoriosOperacionaisApiOnlyCorrigido;
