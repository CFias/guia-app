...
export default function ResumoOperacionalGuias() {
    ...
    const gruposBase = useMemo(() => {
        ...
        somenteComGuia.forEach((item, index) => {
            ...
            const chaveServico = `${passeio}__${veiculo}__${fornecedor}`;
        ...
        return Object.values(mapa)
            .map((grupo) => ({
                ...
                totalServicos: grupo.servicos.length,
                totalPax: grupo.servicos.reduce(
                    (acc, servico) =>
                        acc +
                        servico.reservas.reduce(
                            (sub, reserva) => sub + Number(reserva.quantidade || 0),
                            0
                        ),
                    0
                ),
    ...
    const gruposFiltrados = useMemo(() => {
        if (filtroGuia === "todos") return gruposBase;
        return gruposBase.filter((item) => item.guia === filtroGuia);
    }, [gruposBase, filtroGuia]);

    const resumo = useMemo(() => {
        return {
            guias: gruposBase.length,
            servicos: gruposBase.reduce((acc, item) => acc + item.totalServicos, 0),
            pax: gruposBase.reduce((acc, item) => acc + item.totalPax, 0),
            reservas: gruposBase.reduce(
                (acc, item) =>
                    acc +
                    item.servicos.reduce(
                        (sub, servico) => sub + servico.reservas.length,
                        0
                    ),
                0
            ),
    ...
    const montarResumoTexto = () => {
        const linhas = [
            "Olá, equipe! 🍀",
            "",
            `Lista de passeios em ${formatarDataBr(dataSelecionada)}:`,
            "",
        ];

        gruposFiltrados.forEach((grupo) => {
            grupo.servicos.forEach((servico, index) => {
                // 🚫 IGNORA SERVIÇOS INDESEJADOS
            ...
            <div className="painel-outs-header">
                <div>
                    ...
                    <p className="painel-outs-subtitle">
                        Visualização operacional dos guias escalados com serviços, veículos
                        e reservas do dia.
                    </p>
                </div>
            </div>
            ...
            <div className="painel-outs-grid">
                ...
                <div className="painel-outs-card painel-outs-card-full">
                    <div className="painel-outs-card-header">
                        ...
                        <p>
                            No card fechado: título = guia, subtítulo = passeio. No dropdown:
                            veículos, fornecedor e reservas do guia.
                        </p>
                    </div>
                                            ...
                                            <div className="painel-outs-item-main">
                                                ...
                                                <div className="painel-outs-item-meta resumo-guia-meta">
                                                    ...
                                                    <span className="resumo-chip resumo-chip-muted">
                                                        clique para ver veículos e reservas
                                                    </span>
                                                </div>
                                            </div>
