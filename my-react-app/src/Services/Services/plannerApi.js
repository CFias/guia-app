...
// Nome do guia escalado, direto do roadmap da API do Phoenix (mesmo
// caminho usado no painel operacional de Chegadas/OUT). Prioriza o
// apelido (nickname) e cai pro nome completo quando não houver.
export const extrairGuiaEscalado = (item) =>
...
// ---------------------------------------------------------------------
// Histórico real de escalas: varre as datas informadas direto na API do
// Phoenix (mesma fonte usada no painel operacional) e conta quantos
// serviços cada guia realmente executou, por nome normalizado. Serve
// pra dar equilíbrio de médio prazo na geração de escala — sem depender
// do que ficou salvo no rascunho do planner no Firestore.
// ---------------------------------------------------------------------
export const carregarHistoricoServicosReaisPorNomeGuia = async (datas = []) => {
  ...
  const respostasApi = await Promise.all(
    datasUnicas.map(async (date) => {
      try {
        ...
        console.error(`Erro ao buscar histórico do dia ${date}:`, err);
        return [];
      }
    }),
  );
  ...
  respostasApi.flat().forEach((item) => {
    const nomeGuia = extrairGuiaEscalado(item);
    if (!String(nomeGuia || "").trim()) return;

    const nomePasseio = extrairNomePasseio(item);
    const nomeExibicao = montarNomeServicoExibicao(item);
    if (!nomeExibicao || deveIgnorarServico(nomeExibicao || nomePasseio))
      return;

    const chaveGuia = normalizarTexto(nomeGuia);
    contagemPorNome[chaveGuia] = Number(contagemPorNome[chaveGuia] || 0) + 1;
  });
